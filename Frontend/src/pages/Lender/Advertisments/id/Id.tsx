      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start">
            <CardTitle className="text-2xl font-bold">{ad.shopName}</CardTitle>
            {isOwner && <Badge className="bg-primary">Your Ad</Badge>}
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="relative h-[300px] md:h-[400px] bg-gray-100 rounded-md overflow-hidden">
              {selectedPhoto ? (
                <img src={selectedPhoto || "/placeholder.svg"} alt={ad.shopName}  className="object-cover" />
              ) : (
            ...
            {ad.photos.length > 1 && (
              <div className="flex gap-2 overflow-x-auto pb-2">
                {ad.photos.map((photo, index) => (
                  ...
                  >
                    <img src={photo || "/placeholder.svg"} alt={`Photo ${index + 1}`} className="object-cover" />
                  </div>
                ))}
              </div>
            )}
